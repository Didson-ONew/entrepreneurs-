...
import Game, { setNet } from "./EntrepreneursGame.jsx";
...
function WaitingRoom({ me, lobby, onStart }) {
  const [err, setErr] = useState("");
...
export default function OnlineApp() {
  ...
  const enter = (m) => {
    try { sessionStorage.setItem(STORE_KEY, JSON.stringify(m)); } catch (_) {}
    setMe(m);
  };
  const leave = () => {
  ...
  useEffect(() => {
    ...
    api("/api/resume", { code: saved.code, token: saved.token }).then((r) => {
      ...
      else { try { sessionStorage.removeItem(STORE_KEY); } catch (_) {} }
      setChecking(false);
  ...
  useEffect(() => {
    ...
    es.onmessage = (ev) => {
      ...
      if (msg.type === "lobby") { setLobby(msg); setState(null); }
      else if (msg.type === "state") {
        setConn("live");
        // rebuild the Sets that JSON cannot carry
        const st = msg.state;
        const g = {};
  ...
  if (!state) return <WaitingRoom me={me} lobby={lobby} onStart={() => {}} />;
  ...
  return (
    <>
      <div style={{ position: "fixed", top: 6, right: 8, zIndex: 90, display: "flex", gap: 6, alignItems: "center" }}>
        ...
        <button onClick={leave} title="Leave this game" style={{ fontSize: 10, color: "#6b7280", background: "none", border: "none", cursor: "pointer", textDecoration: "underline" }}>leave</button>
        <span style={{
          fontSize: 10, padding: "2px 7px", borderRadius: 999,
          backgroundColor: conn === "live" ? "#14301f" : "#3a1f1f",
          color: conn === "live" ? "#8fd3b6" : "#fca5a5", border: "1px solid #2b3040",
        }}>{conn === "live" ? "live" : "reconnecting"}</span>
      </div>
      {toast && (
        <div style={{
      ...
      <Game online={{ state, seat: me.seat, logs }} />
    </>
  );
}
