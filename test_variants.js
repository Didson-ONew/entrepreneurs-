...
section("1. Score at the year end");
{
  ...
  st.quarter = 4;
  E.runClosingRest(st, () => {});
  check("the year end pays it", biz.epOnCard === 2 && biz.scored === true, `${biz.epOnCard} EP`);
  ...
  check("whereas standard pays it on the spot", b2.epOnCard === 2 && b2.scored === true);
}
...
section("2. Levels score single");
{
  ...
  check("a level 2 company scores 2 as standard", mk(undefined) === 2, `${mk(undefined)} EP`);
  check("and 6 under the variant", mk({ tripleLevelEP: true }) === 6, `${mk({ tripleLevelEP: true })} EP`);
  check("levelEP says the same", E.levelEP(game(undefined)) === 1 && E.levelEP(game({ tripleLevelEP: true })) === 3);
}
...
section("The bots read the variants too");
{
  ...
  check("a building is worth less to a bot when levels score single",
    scoreOf({ tripleLevelEP: true }) > scoreOf(undefined),
    `${scoreOf(undefined).toFixed(3)} -> ${scoreOf({ tripleLevelEP: true }).toFixed(3)}`);
}
...
section("All five on at once - which is very nearly v12");
{
  ...
  check("a level is worth 3 EP with the triple variant on", E.levelEP(st) === 3);
